...
### End to end steps from the scratch

#### EVM (rusd-evm-contracts)

1.1 `npm run deploy:<chain>` as many as needed
1.2 `npm run wire-oapps` (only for EVM chains)
3.1 `npm run wire-solana` (connect evm chains to solana)

6.1 `forge script Bridge`

#### Solana (rusd-solana-contracts)

2.1 `anchor build -v -e RUSD_OMNICHAIN_ADAPTER_ID=<PROGRAM_ID>`
2.2 `spl-token create-token --program-id TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb --enable-freeze --decimals 6 --enable-metadata --enable-pause --enable-confidential-transfers manual`
2.3 `solana program deploy --program-id target/deploy/oft-keypair.json target/verifiable/oft.so -u <chain> --with-compute-unit-price <COMPUTE_UNIT_PRICE_IN_MICRO_LAMPORTS>`
2.4 `pnpm hardhat lz:oft:solana:create --eid 40168 --program-id <PROGRAM_ID> --mint <TOKEN_MINT> --token-program <TOKEN_PROGRAM_ID> --additional-minters <MINTERS_CSV>`
2.5 `spl-token authorize <TOKEN_MINT> mint <MINT_AUTHORITY> --authority <MINT_AUTHORITY> --multisig-signer ~/.config/solana/id.json`
2.6 `pnpm hardhat lz:oft:solana:set-peer --program-id <PROGRAM_ID> --eid <eid> --peer-eid <eid> --oft-store <OFT_STORE> --peer-address <RECEIVER_BYTES20>` (connect solana to evm)

4.1 `spl-token create-account <TOKEN_MINT>`
4.2 `spl-token mint <TOKEN_MINT> <AMOUNT> --multisig-signer ~/.config/solana/id.json --owner <MINT_AUTHORITY>`

5.1 ` npx hardhat lz:oft:send --src-eid 40168 --dst-eid 40161 --to <RECEIVER_BYTES20> --amount <AMOUNT> --token-program <TOKEN_PROGRAM> --extra-options 0x000301001101000000000000000000000000000493e0`
