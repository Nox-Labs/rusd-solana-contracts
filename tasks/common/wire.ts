...
task(TASK_LZ_OAPP_WIRE)
  ...
  .setAction(async (args: Args, hre, runSuper) => {
    ...
    const sdkFactory = createSdkFactory(
      userAccount,
      programId,
      connectionFactory
    )
    ...
    subtask(
      ...
      "Configure OFT",
      async (
        subtaskArgs: SubtaskConfigureTaskArgs<OAppOmniGraph, IOApp>,
        _hre,
        runSuper
      ) => {
            ...
            if (
              ...
              if (connection.config?.sendLibrary) {
                ...
                if (sendConfig == null || receiveConfig == null) {
                  throw new Error(
                    "SendConfig or ReceiveConfig not initialized. "
                  )
                }
              } else {
                logger.debug(
        ...
        return runSuper({
          ...
          graph: {
            ...
            contracts: subtaskArgs.graph.contracts.filter((contract) => {
              ...
              return (
                chainType !== ChainType.APTOS && chainType !== ChainType.INITIA
              )
            }),
            connections: subtaskArgs.graph.connections.filter((connection) => {
              const fromChainType = endpointIdToChainType(
                connection.vector.from.eid
              )
              return (
                fromChainType !== ChainType.APTOS &&
                fromChainType !== ChainType.INITIA
              )
            }),
          },
        })
...
task(TASK_LZ_OWNABLE_TRANSFER_OWNERSHIP)
  ...
  .setAction(async (args: Args, hre) => {
    return hre.run(TASK_LZ_OAPP_WIRE, {
      ...args,
      internalConfigurator: configureOwnable,
    })
  })
