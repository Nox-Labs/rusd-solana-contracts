...
task("lz:oft:send", "Sends OFT tokens cross‐chain from any supported chain")
  ...
  .addParam(
    "to",
    "Base58 recipient (Solana) or bytes20-encoded target (EVM)",
    undefined,
    types.string
  )
  .addOptionalParam(
  ...
  .addOptionalParam(
    "tokenProgram",
    "Solana Token Program pubkey",
    undefined,
    types.string
  )
  .addOptionalParam(
    "computeUnitPriceScaleFactor",
    "Solana compute unit price scale factor",
    4,
    types.float
  )
  .setAction(async (args: MasterArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (chainType === ChainType.SOLANA) {
      ...
      throw new Error(
        `The chain type ${chainType} is not implemented in sendOFT for this example`
      )
    }
