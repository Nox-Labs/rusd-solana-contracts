  ...
  .setAction(
    async ({
      ...
      if (freezeAuthorityStr && !onlyOftStore) {
        throw new Error(
          "`--freeze-authority` is only supported in `--only-oft-store true` mode"
        )
      }
      ...
      if (
        ...
        additionalMintersAsStrings?.length > 0
      ) {
        throw new Error(
      ...
      if (additionalMintersAsStrings) {
        ...
        console.log(
          `created SPL multisig @ ${mintAuthorityPublicKey.toBase58()}`
        )
      ...
      if (!isMABA) {
        const createV1Args: CreateV1InstructionAccounts &
          CreateV1InstructionArgs = {
          mint,
          name,
          symbol,
      ...
      const accountSize = getAccountLenForMint(mintAccount) // Pass extensions here if needed

      const lockboxSigner = createSignerFromKeypair({ eddsa: eddsa }, lockBox)
      let txBuilder = transactionBuilder()
        .add(
          createAccountWithRent(umi, {
            newAccount: lockboxSigner,
            space: accountSize,
          ...
          .add(
            setAuthority(umi, {
              ...
              newAuthority: freezeAuthority
                ? fromWeb3JsPublicKey(freezeAuthority)
                : null,
