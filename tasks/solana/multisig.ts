...
export async function createMultisig(
  ...
  const lamports =
    await connection.getMinimumBalanceForRentExemption(MULTISIG_SIZE)

  const transaction = new Transaction().add(
    SystemProgram.createAccount({
      fromPubkey: toWeb3JsPublicKey(umiWalletSigner.publicKey),
      newAccountPubkey: multisigPublicKey,
      lamports,
      space: MULTISIG_SIZE,
      programId,
    }),
    ...
    txBuilder = txBuilder.add({
      instruction: {
        keys: instruction.keys.map((key) => ({
          ...key,
          pubkey: umiPublicKey(key.pubkey),
        })),
        programId: umiPublicKey(instruction.programId),
        data: instruction.data,
      },
      signers: [umiWalletSigner, createSignerFromKeypair(umi, keypair)],
  ...
  console.log(
    `createMultisigTx: ${getExplorerTxLink(bs58.encode(tx.signature), isTestnet)}`
  )
...
export const checkMultisigSigners = async (
  ...
  const accountInfo = await assertAccountInitialized(
    connection,
    multisigAddress
  )
