use crate::*;
use anchor_spl::token_interface::{ Mint, TokenInterface, initialize_account3, InitializeAccount3 };
...
impl InitOFT<'_> {
    pub fn apply(ctx: &mut Context<InitOFT>, params: &InitOFTParams) -> Result<()> {
        // Initialize the token escrow account
        initialize_account3(
            CpiContext::new(ctx.accounts.token_program.to_account_info(), InitializeAccount3 {
        ...
        ctx.accounts.oft_store.ld2sd_rate = (10u64).pow(
        ...
        ctx.accounts.oft_store.endpoint_program = if
            let Some(endpoint_program) = params.endpoint_program
        {
            endpoint_program
        } else {
            ENDPOINT_ID
